...
var createTranslate = function createTranslate(local) {
  ...
  return function (key) {
    ...
    if (!(key in languages)) {
      console.error("intl:You need to declare a JSON languages object whose key is KEY: [" + key + "]", "languages: ", languages, "current: ", current);
    } else {
      text = item[current];
      if (text === undefined) {
        text = "undefined";
        console.error("intl: Language not found", "[" + key + "]", "item: ", item, "current: ", current);
      } else if (text === null) {
        text = "null";
        console.error("intl: Language not found", "[" + key + "]", "item: ", item, "current: ", current);
      } else if (typeof text !== "string") {
        console.error("Init: [" + key + "]: value is not a string or number in locales", item, "current: ", current);
        text = "not string";
      }
    }
